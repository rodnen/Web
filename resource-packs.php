    ...
    <div class="mainContent">
        <div class="content flex" style="justify-content:center;">
           <div style="width:760px;">
            ...
            <?$themes = "SELECT * FROM textures_thematics";
              $rows = $this->db->queryRows($themes);
                
                if(isset($_REQUEST['version'])){
                    $link = '?version='.$_REQUEST['version'].'&type=';
                }
                ...
                foreach($rows as $row){
                    ...
                    if($_REQUEST['type'] == $row['id']){
                            echo('<a href="'.$link.$row['id'].'"class="title-padding thematic-active">'.$row['name'].'</a>');    
                    }
                    else{
                        echo('<a href="'.$link.$row['id'].'"class="title-padding thematic">'.$row['name'].'</a>');
                    }
                }
            ...
            ?>
                ...
                <div class="NewPublics background background-shadow" >
                    <div class="title flex"><span>Текстур-паки</span></div>
                        <div class="publics flex"><div class="wrapper-center flex">
                        <?
                        $where = ' WHERE remove_status = "0" AND type = 3';
                        $params = [];
